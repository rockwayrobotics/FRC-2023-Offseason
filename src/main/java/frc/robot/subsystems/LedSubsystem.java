...
public class LedSubsystem extends SubsystemBase {
  ...
  private void aroaceFlag(){
      ...
      for(var i = Math.round(m_ledBuffer.getLength() / 5); i < Math.round(m_ledBuffer.getLength() / 5)*2; i++){
        m_ledBuffer.setRGB(i, 255, 200, 0);
      }
      for (var i = Math.round((m_ledBuffer.getLength()/5))*2; i < Math.round((m_ledBuffer.getLength()/5)*3); i++) {
        m_ledBuffer.setRGB(i, 192, 192, 192);
      }
      for(var i = Math.round((m_ledBuffer.getLength()/5))*3; i < Math.round(m_ledBuffer.getLength() / 5)*4; i++){
        m_ledBuffer.setRGB(i, 0, 170, 255);
      }
      for (var i = Math.round((m_ledBuffer.getLength()/5))*4; i < m_ledBuffer.getLength(); i++) {
